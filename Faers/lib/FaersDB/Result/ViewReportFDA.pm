...
__PACKAGE__->result_source_instance->view_definition(
    q[
    select brand_generic, fda_dt, age, sex, hlgt_name from drug join reac on drug.primaryid = reac.primaryid join demo on drug.primaryid = demo.primaryid join brand on drug.genericid = brand.genericid join outc on reac.primaryid = outc.primaryid join meddra_soc_term on reac.soc_code = meddra_soc_term.soc_code join meddra_hlgt_pref_term on reac.hlgt_code = meddra_hlgt_pref_term.hlgt_code where brand_generic like ? and fda_dt between ? and ? group by soc_name, hlgt_name, brand_generic order by count(pt) desc
    ]
);                                                   
